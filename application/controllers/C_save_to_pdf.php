...
class C_save_to_pdf extends CI_Controller
{
    ...
    public function monitoringAttendanceByRangeDate()
    {
        ...
        $token = $this->input->post('token');
        $data_arr = $this->getInputToken($token);
        $datawarek1 = $this->db->get_where(
            'db_employees.employees em',
        ...
        $pdf->Output('D', 'Monitoring_Attendance_Range_Date.pdf');
    }
...
Phone: (021) 29200456';
    ...
    public function suratMengajar($token)
    {
        ...
        if (count($dataLect) > 0) {
            ...
            // Get PHR -> 2.2
            $dataPHR = $this->db->limit(1)->select('NIP, Name, TitleAhead, TitleBehind, Signature')->get_where(
                'db_employees.employees em',
                "(em.PositionMain = '2.2' OR em.PositionOther1 = '2.2' OR em.PositionOther2 = '2.2' OR em.PositionOther3 = '2.2') AND  em.StatusEmployeeID = '1'"
            )->result_array();
